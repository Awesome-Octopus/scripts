...
whole_pdb = parser.get_structure("randomized_S2E",
                                 "/home/andrew/scripts/simple.pdb")
...
def random_backbone(n_structs, residue_list_filename, cutoff_distance=0.36, anchor='N', max_tries=20):
...
def orient(center_sn, axis_sn):
    ...
    if (len(center_sn) != 1) or (len(axis_sn) != 1) or (center_sn == axis_sn):
        ...
        for n, vect in enumerate(coordinates):
            ...
            coordinates[n] = vect.rotate_arround(-1*yz_ang, z_ax)

        axis_vector = coordinates[axis_sn]
        print(f'after the first rotation the axis vector is: {axis_vector}')
        # now find angle between desired axis and current + z axis
        z_ang = axis_vector.angle_between(vector([0, 0, 1]))[0]
        print(f'the angle reletive to the +z axis is now: {z_ang}')
        # and rotate everything by that
        for n, vect in enumerate(coordinates):

            coordinates[n] = vect.rotate_arround(
                -1*z_ang, vector([1, 0, 0]))
