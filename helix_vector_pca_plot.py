...
for curr_ndx in protein.indices[1:]:
    prev_ndx = chains[-1][-1]
    if all_atoms.resnums[curr_ndx] in {all_atoms.resnums[prev_ndx],  
                                       all_atoms.resnums[prev_ndx] +1}:
...
for i, ts in enumerate(trj.trajectory):
    ...
    if type(args.ref) == tuple:
        ...
        for res in range(int(args.ref[0]), int(args.ref[1])+1):
            ...
            symmetry_avg.append(vector(curr_res.center_of_geometry(
                compound='group')))
    ...
    for j, chain in enumerate(chains):
        ...
        # if i > 0 and helix_vect.dot(hel_vects_list[i-1][j]) < 0:
        #     helix_vect = -helix_vect
        q.append(helix_vect)
        ...
        yaw[i][j] = ang
        # print('pitch:\n',pitch[i][j],'\nyaw\n', yaw[i][j])
    hel_vects_list.append(q)



# <codecell> plotting
if symm_vects_list:
    sym_v_fig = plt.figure()
    symm_vects_list = np.asarray(symm_vects_list)
    ax = sym_v_fig.add_subplot(projection='3d')
    indices = np.arange(len(symm_vects_list[:,0]))
    sca = ax.scatter(symm_vects_list[:,0], symm_vects_list[:,1], 
                     symm_vects_list[:,2], c=indices, cmap='viridis', 
                     label=f'chain {j+1}')
    
    plt.xlabel('x')
    plt.ylabel('y')
    plt.title(f'symmetry axis')
...
hel_v_fig = plt.figure()
hel_vects_list = np.asarray(hel_vects_list)
for j in range(len(chains)):
    if len(chains) > 3:
        ax = hel_v_fig.add_subplot(3, 2, j+1, projection='3d')
    else:
        ax = hel_v_fig.add_subplot(1, len(chains), j+1, projection='3d')
    indices = np.arange(len(hel_vects_list[:,:,0]))

    sca = ax.scatter(hel_vects_list[:,j,0], hel_vects_list[:,j,1], 
                     hel_vects_list[:,j,2], c=indices, cmap='viridis', 
                     label=f'chain {j+1}')

plt.xlabel('x')
plt.ylabel('y')
plt.title('principle helical direction vectors')
plt.colorbar(sca, label='timestep')
