...
function dataObj = eseem_analysis(file)
    ...
    if (isfield(params,'YPTS') == 0); % Checks to see if the data are 1D
        ...
        elseif (isfield(params,'PlsSPELGlbTxt') == 1) % Checks for presence of PulseSPEL Data
            ...
            xf = fdaxis(inc/1000,params.XPTS); % Uses the EasySpin fdaxis function to generate an x axis for the FT
            ft = real(ctafft(ysub,params.XPTS-1,params.XPTS)) % Constructs the Cross Term Averaged FFT of the experimental data
            ft = fftshift(ft); % Shifts the FFT using the Matlab function fftshift to obtain the correct spectrum
        end
...
ft_plot_ax.XLimMode = 'manual';
ft_plot_ax.XLim = [0, 30];
legend(ft_plot_ax);

zoomed_ft = figure;
zoomed_ft_ax = axes(zoomed_ft);
zoomed_ft_ax.Title.String = 'Fourier Transform (zoomed)';
zoomed_ft_ax.XLabel.String = 'Frequency (MHz)';
zoomed_ft_ax.YLabel.String = 'Intensity (a.u)';
zoomed_ft_ax.XLimMode = 'manual';
zoomed_ft_ax.XLim = [1.5, 3];
legend(zoomed_ft_ax);
hold(zoomed_ft_ax, 'on');
...
for i=1:n_groups % for each sample group
    for j=1:data(i).nreps % for each rep
        ...
        plot(scaled_plot_ax, curr.time, curr.scaled_echo, 'Marker', 'none', 'LineStyle', '-', ...
            'Color', data(i).color, 'DisplayName', data(i).title);
        plot(subtracted_plot_ax, curr.time, curr.subtracted_echo, ...
            'Marker', 'none', 'LineStyle', '-', 'Color', data(i).color, 'DisplayName', data(i).title);
        plot(ft_plot_ax, curr.freq, curr.ft, 'Marker', 'none', ...
            'LineStyle','-', 'Color', data(i).color, 'DisplayName', data(i).title);
        plot(zoomed_ft_ax, curr.freq, curr.ft, 'Marker', 'none', ...
            'LineStyle', '-', 'Color', data(i).color, 'DisplayName', data(i).title)
    end
end
