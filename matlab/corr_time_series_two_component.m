...
function [fit, field, data] = corr_time_series_two_component(fname)
    ...
    curr_fit = two_component_nitroxide_single_spectrum(field, data(:,v), params, []);
    
    rmsd = zeros(numel(temps), 1);
    logtcorr = zeros (numel(temps),3);
    for i=1:numel(temps)
        fit(i) = two_component_nitroxide_single_spectrum(field, data(:,i), params, curr_fit);
        curr_fit = fit(i);
